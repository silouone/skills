...
# ingest — source → saved key points (Claude Code adapter)
...
1. **Resolve the source.**
   - YouTube URL or bare video ID → fetch the transcript locally:
     ...
     captions; if none exist, tell the user and stop.
...
3. **Assess applicability** when the content touches the user's tooling or
   workflow (their agent harness, hooks, skills, projects): inspect the
   *actual current state* first — `~/.claude/settings.json`, `~/.claude/skills/`,
   `~/.claude/hooks/`, the current repo — before claiming a gap, and say where
   the user already stands vs. what the source recommends. Recommendations
   must be proportionate — flag what is NOT worth doing too. Skip this section
   entirely if the content has no operational relevance.
