...
# ingest — source → saved key points
...
1. **Resolve the source.**
   - YouTube URL or bare video ID → fetch the transcript locally:
     ...
     captions; if none exist, tell the user and stop. If `uv` is missing, fall
     back to `python3 -m pip install --user youtube-transcript-api`, then run
...
3. **Assess applicability** when the content touches the user's tooling or
   ...
   Inspect the *actual current state* before claiming a gap — whichever of
   these exist on this machine:
   - Claude Code — `~/.claude/settings.json`, `~/.claude/skills/`,
     `~/.claude/hooks/`, `CLAUDE.md`
   - Codex — `~/.codex/config.toml`, `~/.codex/skills/`,
     `~/.codex/automations/`, `AGENTS.md`
   - the current repo, either way

   Then say where the user already stands vs. what the source recommends.
   Recommendations must be proportionate — flag what is NOT worth doing too.
   Skip this section entirely if the content has no operational relevance.
